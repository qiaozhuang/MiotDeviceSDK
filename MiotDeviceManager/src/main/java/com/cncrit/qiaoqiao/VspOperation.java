...
import com.cncrit.qiaoqiao.utils.VspDataUtils;
...
public class VspOperation {
	...
	public static String rsIp = MLContent.FORMAL_SERVER_IP;
	...
	public static DeviceStateOnReceiver deviceStateOnReceiver=null;
...
//				return procGetPuList(vm);
		...
		private boolean procTTBinary(VspMessage vm) {
			...
			if (deviceStateOnReceiver!=null){
				try {
					deviceStateOnReceiver.onReceiverDeviceRes(JSONUitls.getJSONResult(1,"success",
							VspDataUtils.getJSONObject(puId,VspDefine.Id_type_idx, MmwParseUartUtils.byteToStr(MmwParseUartUtils.doLinkBindParse(content)))));
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
			if (vspCallback!=null){
				try {
					vspCallback.onReceiverDeviceUart(puId,content);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
...
//				vspCallback.loginRes(0,cuId,sessionId,domId);
	...
	public static boolean LoginPu(String userName, String password) throws Exception{
		isHeartbeat=true;
		Reset();
		updateLoginActTitle("清理完毕!");
		if (GetCmsAddress()) {
			...
			if (PuLogin(userName,password)) {
					try {
						if (vspCallback!=null) {
						   vspCallback.loginRes(1, "success",VspDataUtils.getJSONObject(puId, sessionId).toString());
						}
					} catch (Exception e) {
						e.printStackTrace();
				}
